...
from prekv.quantizers import QuantizerBase
...
def get_past_key_values(
    ...
    # Llama models have `head_dim` in model config
    ...
    empty_rotary_coeffs = torch.zeros(batch_size, config.num_key_value_heads, 0, head_dim, device=device, dtype=dtype)
